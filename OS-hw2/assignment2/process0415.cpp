...
int main(int argc, char *argv[]){
	...
	for (i12 = 0; i12< NSum+T; i12++){ // T cases' output
		...
		///////cout<<buffer<<endl;
		//if(bufferToString(buffer, sizeof(BUFFER_SIZE)) == to_string((int)(parentid + i17 + 1))){
			//cout<<"yeah"<<endl;
			//finalResult[i17][i18] = 
		//}
		string tempT = bufferToString(buffer, sizeof(BUFFER_SIZE));
		nRead = read(filedes[0], buffer, BUFFER_SIZE);
		string tempN = bufferToString(buffer, sizeof(BUFFER_SIZE));
		//cout<<"herer"<<endl;
		//cout<<stoi(tempT)<<endl;
		...
		//cout<<stoi(tempT)-parentid-1<<endl;
		//cout<<buffer<<endl;
	}
...
void runner(int rT){
	...
	//int temp2 = (int)getpid();
	//char const *pchar3= to_string(temp2).c_str();
        //strcpy(buffer, pchar3);
	//cout<<buffer<<endl;
	//write(filedes[1],buffer, BUFFER_SIZE);
	//endtime = system_clock::now();
	//microseconds duration = duration_cast<microseconds>(endtime - starttime);
	//strcpy(buffer, ((char)(duration.count())).c_str());
	//write(filedes[1],buffer, BUFFER_SIZE);
